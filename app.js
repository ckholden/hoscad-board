...
function _buildReportHtml(rpt) {
  ...
  const sd = rpt.structured_data || {};
  const cadHtml =
    (sd.cadHeader ? '<h2 style="margin:18px 0 6px;">CAD INCIDENT HEADER</h2><div class="narrative">' + h(sd.cadHeader) + '</div>' : '') +
    (sd.cadNotes  ? '<h2 style="margin:18px 0 6px;">CAD INCIDENT NOTES</h2><div class="narrative">'  + h(sd.cadNotes)  + '</div>' : '');
  const addenda = rpt.addenda || [];
  const addendaHtml = addenda.length
    ? '<h2 style="margin:18px 0 6px;">ADDENDA / FOLLOW-UP NOTES</h2>' +
      addenda.map(function(a) {
        return '<div style="margin:8px 0;border-left:3px solid #ccc;padding-left:10px;">' +
          '<div style="font-size:10px;color:#888;">' + h(fmtTs(a.created_at)) + ' — ' + h(a.author_name || '') + '</div>' +
          '<pre style="font-size:12px;font-family:monospace;white-space:pre-wrap;margin:4px 0 0;">' + h(a.note || '') + '</pre>' +
          '</div>';
      }).join('')
    : '';
  return '<!DOCTYPE html><html><head><meta charset="utf-8"><title>' + h(rpt.report_id) + ' — HOSCAD RMS</title>' +
    ...
    'h1{font-size:16px;letter-spacing:.08em;margin:0 0 16px;}h2{font-size:13px;margin:16px 0 6px;letter-spacing:.05em;border-bottom:1px solid #ddd;padding-bottom:4px;}' +
...
async function _dispAttachReportFromModal(reportId, incidentId) {
  const btn = document.getElementById('incAttachBtn-' + reportId);
  if (btn) { btn.disabled = true; btn.textContent = '…'; }
  const r = await API.dispatcherAttachReport(TOKEN, reportId, incidentId);
  if (!r.ok) {
    showAlert('ATTACH FAILED', r.error || 'UNKNOWN ERROR.');
    if (btn) { btn.disabled = false; btn.textContent = 'ATTACH TO INC'; }
    return;
  }
  if (btn) { btn.textContent = '✓ ATTACHED'; }
  showToast('REPORT ' + reportId + ' ATTACHED TO ' + incidentId + '.');
}

window._openReportPopout           = _openReportPopout;
window._clickRptLink               = _clickRptLink;
window._dispAttachReportFromModal  = _dispAttachReportFromModal;
...
// ── Schedule Modal ──────────────────────────────────────────────────────────
var _schedModalDate  = null;   // Date object
var _schedModalUnits = [];
var _schedModalShifts = [];
var _schedModalCrew  = [];
var _schedModalEditUnit = null;
var _schedModalSelCrew  = [];

function _schedModalDateStr(d) {
  return (d instanceof Date ? d : new Date(d)).toISOString().slice(0, 10);
}

function _schedModalFmtTitle(d) {
  return d.toLocaleDateString('en-US', { weekday: 'long', month: 'short', day: 'numeric', year: 'numeric' }).toUpperCase();
}

async function _openScheduleModal(dateStr) {
  var d = dateStr ? new Date(dateStr + 'T12:00:00') : new Date();
  _schedModalDate = d;
  var modal = document.getElementById('scheduleModal');
  modal.style.display = 'flex';
  document.getElementById('schedModalTitle').textContent = _schedModalFmtTitle(d);
  document.getElementById('schedModalBody').innerHTML =
    '<div style="padding:20px;color:var(--muted);text-align:center;font-size:12px;">LOADING…</div>';

  var r = await API.getScheduleData(TOKEN, _schedModalDateStr(d));
  if (!r.ok) {
    document.getElementById('schedModalBody').innerHTML =
      '<div style="padding:20px;color:#ff6b6b;">ERROR: ' + esc(r.error || 'UNKNOWN') + '</div>';
    return;
  }
  _schedModalUnits  = r.units  || [];
  _schedModalShifts = r.shifts || [];
  _schedModalCrew   = r.crew   || [];
  _schedModalEditUnit = null;
  _schedModalSelCrew  = [];
...
function _schedModalNav(dir) {
  var d = new Date(_schedModalDate);
  d.setDate(d.getDate() + dir);
  _openScheduleModal(_schedModalDateStr(d));
}

function _schedModalToday() {
  _openScheduleModal();
}
...
function _renderScheduleModal() {
  ...
  html += '<thead><tr style="background:var(--bg2);border-bottom:1px solid var(--border);">' +
    '<th style="padding:6px 12px;text-align:left;font-size:10px;letter-spacing:.06em;color:var(--muted);">UNIT</th>' +
    '<th style="padding:6px 12px;text-align:left;font-size:10px;letter-spacing:.06em;color:var(--muted);">CREW</th>' +
    '<th style="padding:6px 12px;text-align:left;font-size:10px;letter-spacing:.06em;color:var(--muted);">SHIFT</th>' +
    (canEdit ? '<th style="padding:6px 12px;width:60px;"></th>' : '') +
    '</tr></thead><tbody>';

  _schedModalUnits.forEach(function(unit) {
    var shift = shiftMap[unit.unit_id];
    var isEditing = _schedModalEditUnit === unit.unit_id;
    var crewHtml = '—';
    if (shift && shift.crew_members && shift.crew_members.length) {
      crewHtml = shift.crew_members.map(function(m) {
        return '<span style="display:block;">' + esc(m.full_name) +
          (m.cert_level ? ' <span style="color:var(--muted);font-size:10px;">(' + esc(m.cert_level) + ')</span>' : '') +
          ' <span style="color:var(--muted);font-size:10px;">[' + esc(m.cad_id) + ']</span></span>';
      }).join('');
    }
    var shiftHtml = shift
      ? esc((shift.shift_start || '').slice(0, 5)) + '–' + esc((shift.shift_end || '').slice(0, 5))
      : '—';

    html += '<tr style="border-bottom:1px solid var(--line);">' +
      '<td style="padding:7px 12px;vertical-align:top;white-space:nowrap;font-weight:700;">' + esc(unit.unit_id) +
        (unit.unit_type ? '<br><span style="font-weight:400;font-size:10px;color:var(--muted);">' + esc(unit.unit_type) + '</span>' : '') + '</td>' +
      '<td style="padding:7px 12px;vertical-align:top;">' + crewHtml + '</td>' +
      '<td style="padding:7px 12px;vertical-align:top;white-space:nowrap;color:var(--muted);">' + shiftHtml + '</td>';

    if (canEdit) {
      html += '<td style="padding:7px 12px;vertical-align:top;">' +
        '<button class="btn-secondary" style="font-size:10px;padding:2px 7px;" ' +
        'onclick="_schedModalOpenEdit(\'' + esc(unit.unit_id) + '\')">' +
        (isEditing ? 'CANCEL' : 'EDIT') + '</button></td>';
    }
    html += '</tr>';

    // Inline edit form
    if (isEditing && canEdit) {
      var existingShift = shiftMap[unit.unit_id];
      var selCrewHtml = _schedModalSelCrew.map(function(cid) {
        var m = _schedModalCrew.find(function(c) { return c.cad_id === cid; });
        var label = m ? (m.full_name + (m.cert_level ? ' (' + m.cert_level + ')' : '') + ' [' + cid + ']') : cid;
        return '<span style="display:inline-flex;align-items:center;gap:3px;background:#1a2e1a;color:#7fffb2;' +
          'padding:2px 6px;border-radius:3px;font-size:10px;">' + esc(label) +
          ' <button type="button" style="background:none;border:none;color:#ff6b6b;cursor:pointer;padding:0 1px;font-size:12px;" ' +
          'onclick="_schedModalRemoveCrew(\'' + esc(cid) + '\')">&times;</button></span>';
      }).join(' ');

      html += '<tr style="background:rgba(255,255,255,.02);border-bottom:1px solid var(--border);">' +
        '<td colspan="' + (canEdit ? '4' : '3') + '" style="padding:10px 12px;">' +
        '<div style="font-size:10px;color:var(--muted);margin-bottom:6px;letter-spacing:.05em;">EDITING: ' + esc(unit.unit_id) + ' — ' + _schedModalFmtTitle(_schedModalDate) + '</div>' +
        // Crew search
        '<div style="margin-bottom:6px;">' +
          '<div style="font-size:10px;color:var(--muted);margin-bottom:3px;">CREW</div>' +
          '<div id="smCrewTags" style="display:flex;flex-wrap:wrap;gap:3px;min-height:20px;margin-bottom:5px;">' + (selCrewHtml || '<span style="color:var(--muted);font-size:10px;">No crew assigned.</span>') + '</div>' +
          '<div style="position:relative;">' +
            '<input id="smCrewSearch" placeholder="SEARCH BY NAME OR CAD ID…" autocomplete="off" ' +
            'oninput="_schedModalCrewFilter(this.value)" ' +
            'style="padding:4px 8px;background:#0b1018;border:1px solid var(--line);color:var(--fg);font-family:inherit;font-size:11px;width:100%;box-sizing:border-box;" />' +
            '<div id="smCrewDropdown" style="display:none;position:absolute;top:100%;left:0;right:0;background:#0b1018;border:1px solid var(--line);z-index:300;max-height:140px;overflow-y:auto;"></div>' +
          '</div>' +
        '</div>' +
        // Times
        '<div style="display:flex;gap:8px;margin-bottom:6px;">' +
          '<div><div style="font-size:10px;color:var(--muted);margin-bottom:2px;">START</div>' +
            '<input type="time" id="smStart" value="' + (existingShift ? (existingShift.shift_start || '07:00').slice(0, 5) : '07:00') + '" ' +
            'style="padding:3px 6px;background:#0b1018;border:1px solid var(--line);color:var(--fg);font-family:inherit;font-size:11px;" /></div>' +
          '<div><div style="font-size:10px;color:var(--muted);margin-bottom:2px;">END</div>' +
            '<input type="time" id="smEnd" value="' + (existingShift ? (existingShift.shift_end || '19:00').slice(0, 5) : '19:00') + '" ' +
            'style="padding:3px 6px;background:#0b1018;border:1px solid var(--line);color:var(--fg);font-family:inherit;font-size:11px;" /></div>' +
          '<div style="flex:1;"><div style="font-size:10px;color:var(--muted);margin-bottom:2px;">NOTES</div>' +
            '<input id="smNotes" placeholder="optional" maxlength="200" value="' + esc(existingShift ? (existingShift.notes || '') : '') + '" ' +
            'style="padding:3px 6px;background:#0b1018;border:1px solid var(--line);color:var(--fg);font-family:inherit;font-size:11px;width:100%;box-sizing:border-box;" /></div>' +
        '</div>' +
        // Buttons
        '<div style="display:flex;gap:6px;align-items:center;">' +
          '<button class="btn-good" style="font-size:10px;" onclick="_schedModalSave()">SAVE</button>' +
          (existingShift ? '<button class="btn-secondary" style="font-size:10px;color:#ff6b6b;" onclick="_schedModalClear(' + existingShift.id + ')">CLEAR SHIFT</button>' : '') +
          '<button class="btn-secondary" style="font-size:10px;" onclick="_schedModalCloseEdit()">CANCEL</button>' +
          '<span id="smStatus" style="font-size:10px;color:var(--muted);"></span>' +
        '</div>' +
        '</td></tr>';
    }
  });
  ...
  html += '</tbody></table>';
  if (!_schedModalUnits.length) {
    html = '<div style="padding:20px;color:var(--muted);text-align:center;">NO SCHEDULEABLE UNITS FOUND.</div>';
  }
  document.getElementById('schedModalBody').innerHTML = html;
}

function _schedModalOpenEdit(unitId) {
  if (_schedModalEditUnit === unitId) {
    _schedModalEditUnit = null;
    _schedModalSelCrew = [];
  } else {
    _schedModalEditUnit = unitId;
    var existing = _schedModalShifts.find(function(s) { return s.unit_id === unitId; });
    _schedModalSelCrew = existing ? (existing.crew_cad_ids || []).slice() : [];
  }
  _renderScheduleModal();
  // Focus search after render
  setTimeout(function() {
    var inp = document.getElementById('smCrewSearch');
    if (inp) inp.focus();
  }, 50);
}

function _schedModalCloseEdit() {
  _schedModalEditUnit = null;
  _schedModalSelCrew = [];
  _renderScheduleModal();
}

function _schedModalRemoveCrew(cadId) {
  _schedModalSelCrew = _schedModalSelCrew.filter(function(id) { return id !== cadId; });
  _renderScheduleModal();
}

function _schedModalCrewFilter(q) {
  var dd = document.getElementById('smCrewDropdown');
  if (!dd) return;
  if (!q || q.length < 1) { dd.style.display = 'none'; return; }
  var lower = q.toLowerCase();
  var matches = _schedModalCrew.filter(function(c) {
    return c.full_name.toLowerCase().includes(lower) || c.cad_id.toLowerCase().includes(lower);
  }).slice(0, 10);
  if (!matches.length) { dd.style.display = 'none'; return; }
  dd.innerHTML = matches.map(function(c) {
    return '<div style="padding:5px 8px;cursor:pointer;font-size:11px;" ' +
      'onmouseover="this.style.background=\'#1e2d1e\'" onmouseout="this.style.background=\'\'" ' +
      'onclick="_schedModalAddCrew(\'' + esc(c.cad_id) + '\')">' +
      '<strong>' + esc(c.full_name) + '</strong> ' +
      (c.cert_level ? '<span style="color:var(--muted);">(' + esc(c.cert_level) + ')</span> ' : '') +
      '<span style="color:var(--muted);">[' + esc(c.cad_id) + ']</span></div>';
  }).join('');
  dd.style.display = '';
}

function _schedModalAddCrew(cadId) {
  if (!_schedModalSelCrew.includes(cadId)) {
    _schedModalSelCrew.push(cadId);
  }
  var inp = document.getElementById('smCrewSearch');
  if (inp) inp.value = '';
  var dd = document.getElementById('smCrewDropdown');
...
async function _schedModalSave() {
  var st = document.getElementById('smStatus');
  if (st) st.textContent = 'SAVING…';
  var start = (document.getElementById('smStart') || {}).value || '07:00';
  var end   = (document.getElementById('smEnd')   || {}).value || '19:00';
  var notes = (document.getElementById('smNotes') || {}).value || '';
  var r = await API.setShift(TOKEN, _schedModalDateStr(_schedModalDate),
    _schedModalEditUnit, _schedModalSelCrew, start, end, notes);
  if (!r.ok) { if (st) st.textContent = 'ERROR: ' + (r.error || 'UNKNOWN'); return; }
  // Reload
  var dateStr = _schedModalDateStr(_schedModalDate);
  _schedModalEditUnit = null;
  _schedModalSelCrew = [];
  var r2 = await API.getScheduleData(TOKEN, dateStr);
  if (r2.ok) { _schedModalShifts = r2.shifts || []; }
  _renderScheduleModal();
}

async function _schedModalClear(shiftId) {
  if (!confirm('CLEAR this shift?')) return;
  var r = await API.deleteShift(TOKEN, shiftId);
  if (!r.ok) { showToast('ERROR: ' + (r.error || 'UNKNOWN')); return; }
  var dateStr = _schedModalDateStr(_schedModalDate);
  _schedModalEditUnit = null;
  _schedModalSelCrew = [];
  var r2 = await API.getScheduleData(TOKEN, dateStr);
...
window._openScheduleModal   = _openScheduleModal;
window._closeScheduleModal  = _closeScheduleModal;
window._schedModalNav       = _schedModalNav;
window._schedModalToday     = _schedModalToday;
window._schedModalOpenEdit  = _schedModalOpenEdit;
window._schedModalCloseEdit = _schedModalCloseEdit;
window._schedModalRemoveCrew= _schedModalRemoveCrew;
window._schedModalCrewFilter= _schedModalCrewFilter;
window._schedModalAddCrew   = _schedModalAddCrew;
window._schedModalSave      = _schedModalSave;
window._schedModalClear     = _schedModalClear;
